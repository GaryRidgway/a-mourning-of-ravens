const CONTROL_TOOLTIPS = {
  ...
  enableMobilePoemScale: 'Shrinks the poem type on a narrow screen, where the unscaled size fills a phone with two or three words. Applied before the poem is measured, so a change here moves the type immediately but needs a reload before the colliders and ring geometry agree with it.',
  ...
  enablePinchZoom: 'Let a two-finger pinch scale the poem live on touch devices. A visual transform, not a font-size change, so it is smooth and needs no reload — the collider boxes scale with it and the near-word particles freeze for the gesture, exactly as they do during a scroll. Multiplies on top of Mobile Poem Scale. The live zoom is a display interaction and does not touch the URL; a reload starts back at the loaded size.',
  pinchZoomMin: 'Smallest the pinch can shrink the poem to, as a fraction of the loaded size. 0.4 is two-fifths — enough to pull a whole extra stanza into view on a portrait phone.',
  pinchZoomMax: 'Largest the pinch can grow the poem to, as a fraction of the loaded size. 1.5 leaves room to read in closer than the loaded size without one word filling the screen.',
  enablePinchZoomScrollComp: 'Speeds a swipe up as you zoom out, by dividing the finger delta by the pinch scale so the words track your finger 1:1 at any size — the same swipe covers more of the poem the smaller the type. Off leaves scroll at a constant poem-space speed, so the words crawl on screen when zoomed out. Also compensates the inertia glide.',
